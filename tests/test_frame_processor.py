...
def _build_mock_processor():
    ...
    # Internal state
    proc._frame_count = 0
    proc._cached_depth_map = None
    proc._cached_frame_width = 0
    proc._prev_detection_areas = {}
...
class TestJpegDecode:
    """Test _decode_jpeg method."""

    def test_valid_jpeg(self, dummy_jpeg):
        proc = _build_mock_processor()
        frame = proc._decode_jpeg(dummy_jpeg)
    ...
    def test_corrupt_jpeg_returns_none(self):
        proc = _build_mock_processor()
        frame = proc._decode_jpeg(b"\x00\x01\x02\x03")
...
class TestPipelineRouting:
    ...
    def test_detect_route_calls_yolo(self, dummy_jpeg):
        proc = _build_mock_processor()
        resp = proc.process(dummy_jpeg, MessageType.FRAME_DETECT, frame_id=1)
        assert resp.frame_id == 1
        assert resp.type == "frame_result"
        proc._detector.detect.assert_called_once()

    def test_ocr_route_calls_ocr(self, dummy_jpeg):
        proc = _build_mock_processor()
        resp = proc.process(dummy_jpeg, MessageType.FRAME_OCR, frame_id=2)
        assert resp.ocr_text == "Hello World"
        proc._ocr.read.assert_called_once()

    def test_describe_route_returns_placeholder(self, dummy_jpeg):
        proc = _build_mock_processor()
        resp = proc.process(dummy_jpeg, MessageType.FRAME_DESCRIBE, frame_id=3)
        assert "Gemini" in resp.surroundings

    def test_latency_populated(self, dummy_jpeg):
        proc = _build_mock_processor()
        resp = proc.process(dummy_jpeg, MessageType.FRAME_DETECT)
...
class TestDetectionPipeline:
    ...
    def test_detections_serialized(self, dummy_jpeg):
        proc = _build_mock_processor()
        resp = proc.process(dummy_jpeg, MessageType.FRAME_DETECT)
        assert len(resp.detections) >= 1
        assert resp.detections[0]["class"] == "person"

    def test_depth_enrich_called(self, dummy_jpeg):
        proc = _build_mock_processor()
        ...
        proc._cached_depth_map = np.full((100, 100), 3.0, dtype=np.float32)
        proc.process(dummy_jpeg, MessageType.FRAME_DETECT)
        proc._depth.enrich_detections.assert_called()

    def test_surroundings_populated(self, dummy_jpeg):
        proc = _build_mock_processor()
        resp = proc.process(dummy_jpeg, MessageType.FRAME_DETECT)
...
class TestOCRPipeline:
    ...
    def test_ocr_unavailable_message(self, dummy_jpeg):
        proc = _build_mock_processor()
        proc._ocr.is_available = False
        resp = proc.process(dummy_jpeg, MessageType.FRAME_OCR)
        assert resp.ocr_text == "OCR engine not available."

    def test_ocr_no_text(self, dummy_jpeg):
        proc = _build_mock_processor()
...
class TestMotionDelta:
    ...
    def test_no_previous_frame_no_force(self):
        proc = _build_mock_processor()
        proc._prev_detection_areas = {}
        from src.object_detector import Detection
        dets = [Detection("car", 0.9, (100, 100, 200, 200), 150, 150)]
        force = proc._check_motion_delta(dets)
        assert force is False

    def test_rapid_growth_forces_depth(self):
        proc = _build_mock_processor()
        proc._prev_detection_areas = {"car": 1000}
        from src.object_detector import Detection
    ...
    def test_small_change_no_force(self):
        proc = _build_mock_processor()
        proc._prev_detection_areas = {"person": 10000}
        from src.object_detector import Detection
